...
### 信用卡單筆付款：已用**真實授權**驗證通過

一度誤判為「走不通」。實際上授權成功了，只是**綠界的付款頁不會跳轉**，
回呼在**八分鐘後**才進來 —— 我當時只等了 6~8 秒就下結論。

綠界後台的原始紀錄：

```
OAWC7WB39R0I9ZY4EMTO  2608242017417620  信用卡  授權碼 14563813  卡號末四 1111
付款時間 2026-08-24 20:25:30  已付款  NT$7  手續費 1.00 (2.45%)  淨額 5.00
```

真實授權碼、真實卡號末四碼、真實手續費，**不是模擬付款**（回呼的 `SimulatePaid=0`）。
本服務端：狀態轉 `paid`、`payment_type=Credit_CreditCard`、
事件 `payment.return` 且 `subject_kind=order`。

**教訓：導轉型金流不能用「頁面有沒有跳轉」判斷成功與否。** 真相只在幕後回呼裡。
...
| 項目 | 為什麼 |
|---|---|
| **定期定額的首期扣款（真實授權）** | 同一套操作對單筆信用卡成功過一次，對定期定額試了多次都沒送出去（綠界後台「查無此單」，代表付款根本沒送達）。**間歇性**，非本服務的問題 —— 見下方「刷卡頁為什麼常常送不出去」。 |
...
### 刷卡頁為什麼常常送不出去（成功過一次，其餘都卡住）

| 路徑 | 結果 | 原因 |
|---|---|---|
| 自動化瀏覽器點「立即付款」 | 多半沒反應：Vue 把 `PayForm` 建好、填好、連 `action` 都設好，**就是不送出**（無例外、無網路請求、未呼叫 `submit()`）。**但單筆信用卡成功過一次** | 在他們的壓縮包內；間歇性 |
