...
**不用綠界的授權明細查詢（`CreditDetail/QueryTrade/V2`）來決定動作**：
那支只有正式環境有，而且「先查再送」是兩次往返，而「猜一個、錯了改送另一個」
最多也是兩次、猜對只要一次 —— 查詢並沒有比較省。

（`CreditCheckCode` 已經取得並存進正式環境的 Secret Manager，
設定名 `ECPAY_CREDIT_CHECK_CODE`。目前**沒有任何程式讀它**，
留著是為了將來要做「對帳時顯示綠界端的授權/關帳狀態」——
那件事測試環境驗不了，見「已知風險」。）
...
| 風險 | 說明 |
|---|---|
| **信用卡的綠界端狀態看不到** | `QueryTradeInfo/V5` 只反映付款、**不反映退款與關帳**（實測：退款後它仍回 `TradeStatus=1`、`TradeAmt` 不變）。要看授權/關帳/取消狀態得用 `CreditDetail/QueryTrade/V2`，但那支**只有正式環境有**，加了也無法在 dev 驗證。訂閱那邊有 `ecpay_exec_status` 可看，信用卡目前只能信本地的 `refunded_amount`。 |
